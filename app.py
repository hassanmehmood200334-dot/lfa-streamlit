...
/* Main background */
body {
    background: linear-gradient(135deg,#0f0f17,#1b1b2f,#162447);
...
/* Animation for LFIA Quantitative Detection */
#app-title {
    font-size: 42px;
    font-weight: bold;
    text-align: center;
...
st.markdown("<div id='app-title'>LFIA Quantitative Detection</div>", unsafe_allow_html=True)
...
# =========================================================
dummy_calib = {
    "Tetrodotoxin (TTX)": [ (0.1,0.92),(0.5,0.85),(1,0.78),(5,0.61),(10,0.49),(25,0.32) ],
    "Domoic Acid (DA)": [ (5,0.93),(10,0.86),(25,0.74),(50,0.61),(100,0.50),(250,0.33) ],
    "Okadaic Acid (OA)": [ (1,0.91),(2,0.84),(5,0.73),(10,0.62),(20,0.51),(50,0.30) ]
}

EU_LIMITS = { "Tetrodotoxin (TTX)":44, "Domoic Acid (DA)":20000, "Okadaic Acid (OA)":160 }
...
with st.sidebar:
    ...
    toxin = st.selectbox("Select Toxin", list(dummy_calib.keys()))
...
# Upload Section
# =========================================================
st.markdown("<div class='card'> <h2>Upload LFIA Strip Image</h2></div>", unsafe_allow_html=True)
...
if uploaded_img:
    ...
    st.markdown("<div class='card'><h3>Processed LFIA Strip</h3></div>", unsafe_allow_html=True)
    st.image(cv2.cvtColor(norm, cv2.COLOR_BGR2RGB))
    ...
    if len(peaks) >= 2:
        ...
        dfres = pd.DataFrame({"Line":["Control","Test"],"Raw":[sample_mean_at(norm,ctrl),sample_mean_at(norm,test)],"Corrected":[ctrl_corr,test_corr]})
        st.dataframe(dfres)
        ...
        st.markdown("<h2>Predicted Concentration</h2>", unsafe_allow_html=True)
        st.markdown(f"<h3 style='color:#A5F3FC;'>{conc_pred:.2f} ng/mL</h3>", unsafe_allow_html=True)
        ...
        if ctrl_corr < 5:
            ...
            st.markdown("<h3 style='color:#22c55e;'>NEGATIVE (Below EU limit)</h3>", unsafe_allow_html=True)
...
EU Limit: {EU} ng/mL
"""
        st.download_button("Download Results", data=result_str, file_name="LFIA_results.txt")
